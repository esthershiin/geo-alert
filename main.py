"""
This script short polls clinician statuses every 90 seconds, retrieving the 
...
def main():
    ...
    # url = "https://httpstat.us/503" # for testing http error
    ...
    while True:
        ...
        for clinician_id in range(1, 7):
            ...
            # API call successful
            if response.status_code == 200 and "error" not in json:
                # extract geolocation
                location, bounds = extract_and_convert_geolocation(json)
                ...
                # extract custom error message if any
                ...
                msg = "ClinicianStatus endpoint failed to retrieve the geolocation of the phlebotomist "\
                    "with ID #{}.\n\nUTC Time: {}\nResponse code: {}\nResponse message: {}\n"\
                    .format(clinician_id, timestamp, response.status_code, err_msg)
                send_email(subject, msg)
        
        # short poll API every 90 seconds
        print("Sleeping")
        time.sleep(90)
...
def send_email(subject, msg):
    ...
    sends email via starttls from geoalerts.eshin@gmail.com to
    ...
    """
    mail = MIMEMultipart()
    mail["From"] = "geoalerts.eshin@gmail.com"
    mail["To"] = "sprinter-eng-test@guerrillamail.info"
    mail["Subject"] = subject
    mail.attach(MIMEText(msg))
    password = config('password', default='')  # grab credentials from .env file
    ...
    try:
        ...
        server.login("geoalerts.eshin@gmail.com", password)  # login
        server.sendmail("geoalerts.eshin@gmail.com", "sprinter-eng-test@guerrillamail.info", mail.as_string()) # send email
        print("Email sent.")
